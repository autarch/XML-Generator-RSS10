...

{
    ...
    like( $out, qr{<item[^>]+>.*<content:encoded>.+</content:encoded>.*</item>}s,
          'expect to find content:encoded tag inside item tag' );

    like( $out, qr{<item[^>]+>.*<!\[CDATA\[<foo>bar</foo>\]\]>.*</item>}s,
...

{
    ...
    like( $out, qr{<channel[^>]+>.*<content:items>\s*<rdf:Bag>\s*<rdf:li>.+</rdf:li>\s*</rdf:Bag>\s*</content:items>.*</channel>}s,
          'expect to find content:items -> rdf:Bag -> rdf:li tags nested inside channel tag' );

    like( $out, qr{<rdf:li>.+<content:format\s*rdf:resource=.http://www.w3.org/1999/xhtml.+</rdf:li>}s,
          'expect to find content:format inside rdf:li tag' );

    like( $out, qr{<content:item>.+<rdf:value><!\[CDATA\[<b>Axis</b> Love\]\]></rdf:value>.+</content:item>}s,
          'expect to find CDATA inside content:item tag' );

    like( $out, qr{<content:item>.+<content:format\s*rdf:resource=.http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.+</content:item>}s,
          'expect to find CDATA inside content:item tag' );

    like( $out, qr{<content:item>.+<content:encoding\s*rdf:resource=.http://www.w3.org/TR/REC-xml#dt-wellformed.+</content:item>}s,
          'expect to find CDATA inside content:item tag' );
